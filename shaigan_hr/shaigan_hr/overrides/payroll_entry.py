...
class CustomPayrollEntry(PayrollEntry):
	...
	def make_filters(self):
		filters = frappe._dict(
			...
			custom_territory=self.custom_territory,
			custom_base_town=self.custom_base_town,
	...
	def fill_employee_details(self):
		...
		if not employees:
			...
			if self.custom_bank:
				error_msg += "<br>" + _("Bank: {0}").format(frappe.bold(self.custom_bank))
			if self.custom_territory:
				error_msg += "<br>" + _("Territory: {0}").format(frappe.bold(self.custom_territory))
			if self.custom_base_town:
				error_msg += "<br>" + _("Base Town: {0}").format(frappe.bold(self.custom_base_town))
			if self.custom_city:
				error_msg += "<br>" + _("City: {0}").format(frappe.bold(self.custom_city))
...
def set_fields_to_select(query, fields: list[str] = None):
	default_fields = ["employee", "employee_name", "department", "designation","custom_city","custom_base_town","custom_bank","custom_territory"]
...
def set_filter_conditions(query, filters, qb_object):
	...
	for fltr_key in ["branch", "department", "designation", "grade","custom_city","custom_base_town","custom_bank","custom_territory"]:
