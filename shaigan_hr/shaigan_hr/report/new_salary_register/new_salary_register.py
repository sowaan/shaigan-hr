...
def execute(filters=None):
	...
	loan_data = frappe.db.get_all(
        "Salary Slip Loan",
        filters={"parent": ["in", salary_slip_names]},  
        fields=["parent", "loan_product", "total_payment"]
    )

	loan_map = {}
	for row in loan_data:
		if row.parent not in loan_map:
			loan_map[row.parent] = []
		loan_map[row.parent].append({"loan_product": row.loan_product, "total_payment": row.total_payment})

	loan_products_set = set()
	for loan_entries in loan_map.values():
		for entry in loan_entries:
			if entry["loan_product"]:
				loan_products_set.add(entry["loan_product"])

	loan_products_list = sorted(list(loan_products_set)) 



	earning_types, ded_types = get_earning_and_deduction_types(salary_slips)
	
	# loans = get_loan_types()
	columns = get_columns(earning_types, ded_types, loan_products_list)
	...
	for ss in salary_slips:
		row = {
			...
			"monthly_salary": ss.custom_monthly_salary,
		...
		for loan_product in loan_products_list:
			total_payment = 0
			if ss.name in loan_map:
				for loan_entry in loan_map[ss.name]:
					if loan_entry["loan_product"] == loan_product:
						total_payment = loan_entry["total_payment"]
			row[frappe.scrub(loan_product)] = total_payment
		...
		if currency == company_currency:
			...
			row.update(
				{"gross_pay": ss.gross_pay, "total_deduction": flt(ss.total_deduction) + flt(ss.total_loan_repayment), "net_pay": ss.net_pay}
			)
...
def get_columns(earning_types, ded_types, loan_products):
	columns = [
		{
	...
	for loan_product in loan_products:
		columns.append(
			{
				"label": loan_product,
				"fieldname": frappe.scrub(loan_product),
				"fieldtype": "Currency",
				"options": "currency",
				"width": 120,
			}
